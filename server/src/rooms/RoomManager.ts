...
export class RoomManager {
  ...
  // Host can kick a player before game starts
  kickPlayer(hostSocketId: string, targetPlayerId: string): GameRoom | null {
    const roomId = this.playerRooms.get(hostSocketId);
    if (!roomId) return null;
    const room = this.rooms.get(roomId);
    if (!room) return null;
    if (room.hostId !== hostSocketId) return null;
    if (room.gameState && room.gameState.phase === 'playing') return null; // Can't kick after game starts
    // Remove player
